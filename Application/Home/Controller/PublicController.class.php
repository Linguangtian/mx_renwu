    ...
    public function getcode()
  {
     ...
     if( !$this->checkMobile( $tel ) )
     {
      ...
      $statusStr = array(
        ...
        $content= "【超热点】您的验证码为{$code}，验证码5分钟内有效。";
    ...
    public function reg()
    {
        ...
        if( IS_POST ) {
                 ...
                 if( empty( $code ) ) 
            {
             ...
             if( $code.$username != session('yzmcode')  )
            {
               $this->error('短信验证码不正确。');
            }
            ...
            if( $res ) {
                ...
                session('member', $user);
                $this->success('注册成功', sp_cfg('app_download_url'));
            } else {
                $this->error('注册失败');
            }
        } else {
